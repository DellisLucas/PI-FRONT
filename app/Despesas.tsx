...
export default function DespesasScreen() {
  ...
  const carregarDespesas = async () => {
    try {
      ...
      const response = await expenses.list(user.id);
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled) {
      setNovaDespesa(prev => ({ ...prev, imagem: result.assets[0].uri }));
    }
  };

  const removeImage = () => {
    setNovaDespesa(prev => ({ ...prev, imagem: null }));
  };

  const handleRowPress = (despesa: any) => {
  ...
  const handleEdit = () => {
    ...
    const [year, month, day] = selectedDespesa.date.split('T')[0].split('-').map(Number);
    const date = new Date(year, month - 1, day);
    setNovaDespesa({
      data: formatDate(date),
      descricao: selectedDespesa.description,
      valor: selectedDespesa.amount.toString(),
      categoria: selectedDespesa.category,
      imagem: selectedDespesa.image || null,
    });
  ...
  const handleDelete = async () => {
    try {
      if (!selectedDespesa || !selectedDespesa._id) throw new Error('ID da despesa não encontrado');
      await expenses.delete(selectedDespesa._id);
      setActionModalVisible(false);
      carregarDespesas();
      Toast.show({ type: 'success', text1: 'Despesa excluída', text2: 'A despesa foi excluída com sucesso!' });
    } catch (error) {
      Toast.show({ type: 'error', text1: 'Erro', text2: 'Não foi possível excluir a despesa' });
    }
  };
  ...
  const handleDateChange = (_: any, selectedDate?: Date) => {
    ...
    if (selectedDate) {
      ...
      setNovaDespesa(prev => ({ ...prev, data: formatDate(localDate) }));
    }
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const despesaData = {
        description: novaDespesa.descricao.trim(),
        amount: valorNumerico,
        date: formattedDate,
        category: novaDespesa.categoria.trim(),
        image: novaDespesa.imagem,
      };

      if (!selectedDespesa || !selectedDespesa._id) {
        await expenses.create(despesaData);
      } else {
        await expenses.update(selectedDespesa._id, despesaData);
      }

      setModalVisible(false);
      setNovaDespesa({ data: '', descricao: '', valor: '', categoria: '', imagem: null });
      setSelectedDespesa(null);
      await carregarDespesas();

      Toast.show({ type: 'success', text1: 'Despesa salva', text2: 'A despesa foi salva com sucesso!' });
    } catch (error: any) {
      ...
      Toast.show({ type: 'error', text1: 'Erro', text2: error.message || 'Não foi possível salvar a despesa' });
    }
  };
  ...
  const formatValue = (text: string) => {
    ...
    setNovaDespesa(prev => ({ ...prev, valor: formatted }));
  };
      ...
      <Container>
        ...
        <Table>
          ...
          {despesas.map((item, index) => {
            const isLast = index === despesas.length - 1;
      ...
      >
        <ModalOverlay>
          <ModalContent>
            ...
            {showDatePicker && (
              <DateTimePicker
                value={novaDespesa.data ? new Date(novaDespesa.data.split('-').reverse().join('-')) : new Date()}
                mode="date"
            ...
            <ImageContainer>
              {novaDespesa.imagem ? (
                <View>
                  <ImagePreview source={{ uri: novaDespesa.imagem }} />
